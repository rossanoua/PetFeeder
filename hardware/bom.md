...
| 8b | **Bulk hopper — funnel** | 3D printed, parametric — `bulk_hopper_module.scad part="funnel"`. Ø160 round top × **140 mm** tall, narrows to **rect bottom matching the cap inlet hole (35×35 rounded)**. **No internal cone** (split out into spider, 9b). **No spout walls** (collar-mount, sits ON the cap). **Prints without supports** when oriented round-opening-down. | 1 | 🧪 to validate |
| 8b-2 | **Bulk hopper — spider** (anti-bridge insert) | 3D printed, parametric — `bulk_hopper_module.scad part="spider"`. Central tip-UP cone (Ø40 base, 35 mm) + 4 radial ribs whose outer ends match the funnel cavity at z=47.5..82.5. **Drops into the funnel from above, settles by friction** at the design z. Prints flat (cone base on bed), **no supports** required. | 1 | 🧪 to validate |
