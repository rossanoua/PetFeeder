...
| 5a | **Paddle wheel** (primary) | 3D printed, parametric — `cad/openscad/paddle_wheel_module.scad`. Full-height hub + N paddles on the bottom half (sweep food along the floor). **2026-05-28 redesign:** wheel_d 80→**120 mm**, wheel_thickness 18→**10 mm**, hub_d 20→**30 mm**. Pocket ≈ 35 mL ≈ **14 g portion**. See ADR `2026-05-28-wheel-upscale-bigger-hole`. | 1–2 | 🧪 to validate |
| 5b | Paddle-wheel housing | 3D printed. **Lies on its flat floor.** Closed bottom with a rounded-rect OUTLET hole (**35×35 mm**, corner r=2); OPEN top; rim recess for the cap. Housing Ø **127.6 mm** × **15.5 mm**. | 1 | 🧪 to validate |
| 5c | Paddle-wheel end cap | 3D printed, removable lid. Slip-fit register lip into housing rim; central axle bore + radial-offset INLET hole (**35×35 mm rounded**) + **COLLAR** (low rectangular fence) on top of the cap that retains the hopper laterally (collar-mount, no socket — see ADR `2026-05-25-collar-mount-hopper-redesign`). | 1 | 🧪 to validate |
...
| 8b | **Bulk hopper — funnel** | 3D printed, parametric — `bulk_hopper_module.scad part="funnel"`. Ø160 round top × **110 mm** tall (opened up from 140 mm to ~31° wall angle from vertical, well below kibble mass-flow limit ~33°), narrows to **rect bottom matching the cap inlet hole (35×35 rounded)**. **No internal cone** (split out into spider, 9b). **No spout walls** (collar-mount, sits ON the cap). **Print right-side up — rect plug on the bed, lip pointing up.** Walls slope outward at 31° (printable as continuous overhang); lip-cone outer transition is an INWARD step (not an overhang). | 1 | 🧪 to validate |
| 8b-2 | **Bulk hopper — spider** (anti-bridge insert) | 3D printed, parametric — `bulk_hopper_module.scad part="spider"`. Central tip-UP cone (Ø40 base, 35 mm) + 4 radial ribs whose outer ends match the funnel cavity at the spider's design z (auto-adjusts to funnel_h). **Drops into the funnel from above, settles by friction** at the design z. Prints flat (cone base on bed), **no supports** required. | 1 | 🧪 to validate |
...
| 9a | **Lower body chassis** | 3D printed, `chassis_module.scad`. **240×200×160 mm** (depth grew 220→240 to host the bigger Ø160 bulk hopper without rear overhang). C-shape: bowl niche at front (130×160×80 mm), top recess seating the rotary housing (**Ø128**), internal slanted chute redirecting kibble from floor outlet to a front-face exit above the bowl, electronics-bay cavity carved from the back. | 1 | 🧪 to model further (next: motor mount, panel cutouts, load-cell mount) |
