...
| 5a | **Paddle wheel** (primary) | 3D printed, parametric — `cad/openscad/paddle_wheel_module.scad`. Full-height hub + N paddles on the bottom half (sweep food along the floor). wheel_d **80**, thickness **18**, hub_d **20 mm**. **2026-06-05: ACTIVE STIRRER CONE on the hub top** — a frustum + 3 proud vanes (paddle continuations) under the (axle-ward) inlet; rotating, the vanes break any central arch + no flat surface for kibble to rest on. Portion set by **rotation count + HX711**, not pocket volume. Print cone-up (no supports). | 1–2 | 🧪 to validate |
| 5b | Paddle-wheel housing | 3D printed. **Lies on its flat floor.** Closed bottom with a rounded-rect OUTLET hole (**33×34 mm**, corner r=2, max on the Ø80 wheel); OPEN top; **4 flush rim MERLONS** (raised wall sections, within the wall OD) that key the cap (anti-rotation, prints support-free). Housing Ø **87.6 mm** × **37 mm**. | 1 | 🧪 to validate |
| 5c | Paddle-wheel end cap | 3D printed, removable lid. Drops onto the housing rim; **4 edge NOTCHES** that the housing merlons poke through (anti-rotation, 4 index positions, no protruding features, no supports). Central axle bore + radial-offset INLET hole (**33×34 mm rounded, extended toward the axle**) + **COLLAR** (low rectangular fence) on top of the cap that retains the hopper laterally (collar-mount, no socket — see ADR `2026-05-25-collar-mount-hopper-redesign`). | 1 | 🧪 to validate |
...
| 8b | **Bulk hopper — funnel** | 3D printed, parametric — `bulk_hopper_module.scad part="funnel"`. Ø160 round top × **115 mm** tall, narrows to **rect bottom matching the cap inlet hole (33×34 rounded)**. Cone is **more open (~36° from vertical)** with a **straight Ø160 cylinder above** — bigger throat + wider approach to reduce arching. Primary active anti-bridge is now the **wheel's stirrer cone** (5a); a **vibromotor** is still planned but its mount is TBD (removed for now, re-added with the real motor). **No spider, no spout walls** (collar-mount). **Print right-side up — rect plug on the bed, lip up.** | 1 | 🧪 to validate |
| 8c | **Bulk hopper — storage ring** | 3D printed, parametric — `bulk_hopper_module.scad part="ring"`. Ø160 × **150** mm (printed part is 160 mm with its stacking lip — 170 would have been 180 mm = the whole bed, no brim margin). Print **1** for ~1.5 kg total capacity (funnel 0.9 L + ring 2.79 L = 3.69 L × 0.4 g/mL), **2** for ~2.6 kg. Modular — print one extra ring to recover the old volume. Stackable slip-fit lip. | 1+ | 🧪 to validate |
...
| 9a | **Lower body chassis** | ~~`chassis_module.scad`~~ **DEPRECATED / archived** (`archive/chassis_module.scad`). The body is now the Ø160 cylinder tower: `bulk_hopper_module.scad` `base_motor` + `base_hopper` (2 snap-locked parts) standing FLAT on its own sole (form 1a — the screw-in legs and the external weighing station were removed 2026-07-13). Electronics live in the base annulus: `el_tray` (vertical, on rails) + `el_panel` (service window, DC jack + USB). Feeding: a shallow pull-out `tray` (Ø150 × 16, ~280 ml) rides `cell_platform` on the load cell, in the front scallop. | — | ✅ superseded |
